    ...
    return (
        <div className='shadow-md w-full fixed top-0 left-0 z-50'>
            <div className='md:px-10 py-4 px-7 md:flex justify-between items-center opacity-90' style={{ backgroundColor: '#00A09A' }}>
                ...
                {/* menu icons */}
                <div onClick={() => setIsOpen(!isOpen)} className='w-12 h-12 text-white cursor-pointer absolute right-8 top-6 md:hidden duration-300 '>
                ...
                <ul className={`md:flex pl-9 md:pl-0 md:items-center justify-center md:static md:pb-0 pb-12 absolute md:z-auto z-[-1] left-0 w-full md:w-auto transition-all duration-500 text-white ease-in ${isOpen ? 'top-12' : 'top-[-490px]'}`} style={{ backgroundColor: '#00A09A' }}>
                    {Links.map((link, index) => (
                        <li className='font-semibold my-7 md:my-0 md:ml-8 relative hover:text-yellow-300 duration-500  ease-out' key={index}>
                            {link.name === 'Ranking' ? (
                    ...
                    <li className='font-semibold my-7 md:my-0 md:ml-8'>
                        <Link to='/signup' className='text-white bg-yellow-400 hover:bg-yellow-500 py-3 px-3 rounded-md'>Sign Up</Link>
                    </li>
                </ul>
            </div>
        </div>
